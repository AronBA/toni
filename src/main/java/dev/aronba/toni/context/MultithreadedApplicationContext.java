...
public class MultithreadedApplicationContext implements ApplicationContext {
  ...
  public MultithreadedApplicationContext() {
    this.instances = new ConcurrentHashMap<>();
    this.dependencyGraph = new ConcurrentHashMap<>();
    this.componentPostProcessors = new CopyOnWriteArrayList<>();
  }
  ...
  private void runPostProcessors() {
    instances.entrySet().parallelStream()
        .forEach(
            entry ->
                componentPostProcessors.parallelStream()
                    .forEach(
                        processor ->
  ...
  private List<Class<?>> sortGraph() throws CircularDependencyException {
    List<Class<?>> sorted = new CopyOnWriteArrayList<>();
    Set<Class<?>> visited = new CopyOnWriteArraySet<>();
    Set<Class<?>> visiting = new CopyOnWriteArraySet<>();
